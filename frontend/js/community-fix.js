...
const _LOCAL_BANNED = [
  ...
  'ابن الحرام','بنت الحرام','خنزير','خنزيره','لعين','ملعون','ملعونه',
  'فاشل','فاشله','تافه','تافهه','مجنون','مجنونه','حقير','حقيره',
  'وسخ','وسخه','نذل','نذله','وقح','وقحه','سافل','سافله',
  'متخلف','متخلفه','بليد','بليده','ابله','ابلهه','معتوه',
  'اكرهك','اكرهج','اخرس','اخرسي','اسكت','اسكتي',
];

function _normalize(t) {
  return (t||'').replace(/[إأآا]/g,'ا').replace(/ى/g,'ي').replace(/ة/g,'ه').replace(/\s+/g,' ').trim();
}

function _localBannedCheck(text) {
  const t = _normalize(text);
  return _LOCAL_BANNED.some(w => t.includes(_normalize(w)));
}

// Expose to window so dashboard.html can override after load (dashboard wins if it runs later)
if (!window.containsBannedWord) {
  window.containsBannedWord = _localBannedCheck;
}
...
async function moderateText(text) {
  const bannedFn = window.containsBannedWord || _localBannedCheck;
  if (bannedFn(text)) {
...
function formatTime(value) {
  try {
    if (value?.toDate) {
      return value.toDate().toLocaleTimeString("ar-SA", { hour: "2-digit", minute: "2-digit" });
    }
    if (value) {
...
function typeLabel(type) {
  return (
    { secondary: "طالب ثانوي", university: "طالب جامعي", mentor: "مرشد أكاديمي", student: "طالب" }[type] ||
    type || "مستخدم"
  );
}
...
function typeColor(type) {
  return (
    { secondary: "var(--gold)", university: "var(--green-main)", mentor: "#1a3a6a", student: "var(--green-main)" }[type] ||
    "var(--green-main)"
  );
}
...
window.loadCommunityUsersFromFirebase = async function () {
  ...
  if (!user) return [];
  try {
    const q = query(collection(db, "users"), where("city", "==", user.city || ""));
...
window.loadPostsWallFromFirebase = async function () {
  ...
  if (!wall || !user) return;
  try {
    const q = query(collection(db, "posts"), where("city", "==", user.city || ""), limit(30));
    const snap = await getDocs(q);
    const posts = [];
    snap.forEach((d) => posts.push({ id: d.id, ...d.data() }));
    posts.sort((a, b) => getTimestampMs(b.createdAt || b.time) - getTimestampMs(a.createdAt || a.time));
    if (!posts.length) {
      wall.innerHTML = '<p style="color:var(--text-muted);text-align:center;padding:16px;font-size:14px;">لا توجد منشورات بعد — كن أول من ينشر! 📢</p>';
      return;
    }
    wall.innerHTML =
...
window.showRequestModal = async function () {
  ...
  sel.innerHTML = users
    .map((u) => `<option value="${escapeHtml(u.name)}" data-uid="${escapeHtml(u.uid)}">${escapeHtml(u.name)} (${escapeHtml(typeLabel(u.type))})</option>`)
    .join("");
  sel.onchange = () => {
...
window.sendRequest = async function () {
  ...
  if (!user || !myUid) { showToast?.("سجلي الدخول أولاً", "error"); return; }
  if (!target || !targetUid) { showToast?.("اختاري شخصاً صحيحاً أولاً", "error"); return; }
  if (targetUid === myUid) { showToast?.("لا يمكنك إرسال طلب لنفسك", "error"); return; }
  if (msg.length > 2) {
    showToast?.("⏳ جاري فحص الرسالة...", "info");
    const check = await moderateText(msg);
    if (check.blocked) { showToast?.(`⚠️ ${check.reason}، يرجى تعديل الرسالة`, "error"); return; }
  }
  try {
    ...
    closeRequestModal();
    addNotification?.("طلب إرشاد مُرسل ✅", `تم إرسال طلبك إلى ${target}`, "info");
...
window.listenGuidanceRequests = function () {
  ...
  return onSnapshot(q,
    ...
    snap.forEach((d) => {
      ...
      if ((data.msgType === "guidance_request" || data.msgType === "reply_request") && data.status === "pending") requests.push(data);
    });
...
// ── قبول طلب الإرشاد ──────────────────────────────────────────
window.approveGuidanceRequest = async function (requestId, studentUid, studentName, studentType, studentCity, gpa, semester, university, spec) {
  const user = getCurrentUser?.();
  const myUid = user?.uid || user?.email || "";
  if (!myUid || !studentUid) { showToast?.("تعذر بدء المحادثة", "error"); return; }
  try {
    const convKey = makeConvKey(myUid, studentUid);
    await updateDoc(doc(db, "messages", requestId), { status: "approved", read: true });
    await addDoc(collection(db, "messages"), {
      msgType: "chat",
      convKey,
    ...
    const savedStudents = JSON.parse(localStorage.getItem('myStudents__' + myUid) || '[]');
    if (!savedStudents.find(s => s.uid === studentUid)) {
      savedStudents.push({ uid: studentUid, name: studentName, type: studentType, city: studentCity, gpa, semester, university, spec });
      localStorage.setItem('myStudents__' + myUid, JSON.stringify(savedStudents));
      ...
      if (!legacy.find(s => s.uid === studentUid)) {
        legacy.push({ uid: studentUid, name: studentName, type: studentType, city: studentCity });
    ...
    if (typeof showStudentProfile === "function") {
      showStudentProfile(studentUid, studentName, studentType, studentCity, gpa, semester, university, spec);
    }
  } catch (e) {
...
window.rejectGuidanceRequest = async function (requestId) {
  try {
    await updateDoc(doc(db, "messages", requestId), { status: "rejected", read: true });
...
window.openRealChat = function (otherUid, otherName) {
  const user = getCurrentUser?.();
  const myUid = user?.uid || user?.email || "";
  if (!myUid || !otherUid) { showToast?.("تعذر فتح المحادثة", "error"); return; }
  const convKey = makeConvKey(myUid, otherUid);
  activeChatOtherUid = otherUid;
  activeChatOtherName = otherName || "محادثة";
  const mc = document.getElementById("mainContent");
  if (!mc) { showToast?.("لم يتم العثور على مساحة عرض المحادثة", "error"); return; }
  mc.innerHTML = `
    <div class="chat-page">
  ...
  activeChatUnsubscribe = onSnapshot(q, (snap) => {
    ...
    messages.sort((a, b) => getTimestampMs(a.createdAt) - getTimestampMs(b.createdAt));
...
window.sendRealChatMessage = async function () {
  ...
  if (!text) return;
  if (!myUid || !activeChatOtherUid) { showToast?.("لا يوجد مستلم للمحادثة", "error"); return; }
...
window.listenMyConversations = function () {
  const user = getCurrentUser?.();
  const myUid = user?.uid || user?.email || "";
  if (!myUid) { showToast?.("سجلي الدخول أولاً", "error"); return null; }
...
window.updatePostsBadge = async function () {
  ...
  try {
    ...
    snap.forEach((d) => {
      const m = d.data();
      if (!m.read && (m.msgType === "guidance_request" || m.msgType === "chat")) count++;
    });
    const badge =
